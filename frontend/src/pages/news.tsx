...
const mockNews: NewsItem[] = [
  {
    ...
    summary:
      "Jerome Powell announces quarter-point rate reduction citing economic stability and inflation targets. The decision was unanimous among voting members.",
  ...
  },
  {
    ...
    summary:
      "BlackRock's Bitcoin ETF records $2.1B in daily inflows as institutional adoption accelerates. Largest single-day flow since launch.",
  ...
  },
  {
    ...
    summary:
      "European Union completes Markets in Crypto-Assets framework, setting compliance standards for 2025. Major exchanges must comply by June.",
  ...
  },
  {
    ...
    summary:
      "Over 30 million ETH now staked in Ethereum 2.0 network, representing 25% of total supply. Validator count hits new record.",
  ...
  },
  {
    ...
    summary:
      "Traditional markets surge following Fed announcement, with tech stocks leading gains. NASDAQ up 1.8% in after-hours trading.",
  ...
  },
  {
    ...
    summary:
      "Major exchange announces Lightning Network support for Bitcoin deposits and withdrawals. Feature rolling out to all users by Q1 2025.",
      ...
      <div className={styles.container}>
        <main className={styles.main}>
          ...
          {!loading && filteredNews.length === 0 && newsData.length > 0 && (
            <div className={styles.endOfResults}>
              No {activeTab} articles found. Try a different category or
              &quot;All News&quot;.
            </div>
          )}
            ...
            <div className={styles.summaryGrid}>
              <div className={styles.summaryCard}>
                ...
                <div className={styles.summaryPrice}>
                  $
                  {pricesData?.bitcoin?.price
                    ? pricesData.bitcoin.price.toLocaleString()
                    : "96,582"}
                </div>
                <div
                  className={`${styles.summaryChange} ${
                    (pricesData?.bitcoin?.change24h || 2.34) >= 0
                      ? styles.positive
                      : styles.negative
                  }`}
                >
                  {pricesData?.bitcoin?.change24h
                    ? `${
                        pricesData.bitcoin.change24h >= 0 ? "+" : ""
                      }${pricesData.bitcoin.change24h.toFixed(2)}%`
                    : "+2.34%"}
                </div>
              </div>
              <div className={styles.summaryCard}>
                ...
                <div className={styles.summaryPrice}>
                  $
                  {pricesData?.ethereum?.price
                    ? pricesData.ethereum.price.toLocaleString()
                    : "3,421"}
                </div>
                <div
                  className={`${styles.summaryChange} ${
                    (pricesData?.ethereum?.change24h || 1.89) >= 0
                      ? styles.positive
                      : styles.negative
                  }`}
                >
                  {pricesData?.ethereum?.change24h
                    ? `${
                        pricesData.ethereum.change24h >= 0 ? "+" : ""
                      }${pricesData.ethereum.change24h.toFixed(2)}%`
                    : "+1.89%"}
                </div>
              </div>
              <div className={styles.summaryCard}>
                ...
                <div className={styles.summaryPrice}>
                  $
                  {pricesData?.solana?.price
                    ? pricesData.solana.price.toLocaleString()
                    : "180"}
                </div>
                <div
                  className={`${styles.summaryChange} ${
                    (pricesData?.solana?.change24h || 1.2) >= 0
                      ? styles.positive
                      : styles.negative
                  }`}
                >
                  {pricesData?.solana?.change24h
                    ? `${
                        pricesData.solana.change24h >= 0 ? "+" : ""
                      }${pricesData.solana.change24h.toFixed(2)}%`
                    : "+1.2%"}
                </div>
              </div>
              <div className={styles.summaryCard}>
                ...
                <div className={styles.summaryPrice}>
                  $
                  {pricesData?.hyperliquid?.price
                    ? pricesData.hyperliquid.price.toFixed(2)
                    : "25.42"}
                </div>
                <div
                  className={`${styles.summaryChange} ${
                    (pricesData?.hyperliquid?.change24h || 0.5) >= 0
                      ? styles.positive
                      : styles.negative
                  }`}
                >
                  {pricesData?.hyperliquid?.change24h
                    ? `${
                        pricesData.hyperliquid.change24h >= 0 ? "+" : ""
                      }${pricesData.hyperliquid.change24h.toFixed(2)}%`
                    : "+0.5%"}
                </div>
              </div>
            </div>
