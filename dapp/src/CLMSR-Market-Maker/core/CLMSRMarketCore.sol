    ...
    ReentrancyGuardUpgradeable
{
    ...
    uint256 public constant FEE_RANGE = 1e4;
    ...
    function openPosition(
        ...
        uint256 cost6WithFees = cost6 * (FEE_RANGE + fee) / FEE_RANGE;

        uint256 fees = cost6WithFees - cost6;

        
        require(cost6WithFees <= maxCost, CE.CostExceedsMaximum(cost6, maxCost));
        
        // Transfer payment from caller (msg.sender)
        _pullUSDC(msg.sender, cost6WithFees);
        
        _pushUSDC(treasury, fees);
    ...
    function decreasePosition(
        ...
        uint256 proceedsMinusFees = proceeds * (FEE_RANGE - fee) / FEE_RANGE;

        uint256 fees = proceeds - proceedsMinusFees;
        
        require(proceedsMinusFees >= minProceeds, CE.ProceedsBelowMinimum(proceeds, minProceeds));
        ...
        _pushUSDC(msg.sender, proceedsMinusFees);
    ...
    function calculateOpenCost(
        ...
        uint256 grossCost = costWad.fromWadRoundUp();

        uint256 netCost = (grossCost * (FEE_RANGE + fee)) / FEE_RANGE;
    ...
    function calculateDecreaseProceeds(
        ...
        uint256 grossProceeds = proceedsWad.fromWadRoundUp();

        uint256 netProceeds = grossProceeds * (FEE_RANGE - fee) / FEE_RANGE;
    ...
    function calculateQuantityFromCost(
        ...
        require(
         ...
         uint256 cost = (costPlusFees * FEE_RANGE) / (FEE_RANGE + fee); 
    ...
    function setFeeRate(uint256 _feeRate) external onlyOwner {
      require(_feeRate > 0 && _feeRate <= FEE_RANGE, "Fee rate out of allowed range");
